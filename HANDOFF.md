...
> ## CURRENT STATE — read this block first (written 2026-09-19, end of the session that built the bootstrap layer; **updated 2026-09-19 by the `new-run` session at `8b3b416`** — see the clean-room line below and open items 1-2)
...
> **Repos (public — Wilson made them public on purpose):** `github.com/lordwilsonDev/fcve` (this repo, `main`), `github.com/lordwilsonDev/ico-collatz-verification` (VCE-002 Lean project, commit `409c4c3`). Upstream PRs open, not merged: `leanprover/lean4export#52`, `ammkrn/nanoda_lib#36`.
...
> **Verified so far:** smoke test ~46-48 s; **75** runtime tests (57 at `8b3b416`) (2 slow-tier skipped — `run-tests.sh --full` not yet run); clean room = 3 FAIL (found BUG-008) then PASS on `5c825cf` ×3, `42f439a` ×1, and **`8b3b416` ×2 (2026-09-19, the `new-run` session)** — **but** tools were *adopted* (`--reuse-from`, disk too tight to build from scratch) and **no fresh Claude session has done a reconstruction**. Hermes loading verified with its own `prompt-size` + scanners; FreeBuff live behavior is **unverified** (no non-interactive mode).
> **Clean room is disk-gated (2026-09-19, `new-run` session).** At `8b3b416` the first pair of runs FAILED and the cause was *not* the code: the only failing check was the doctor's `storage room to finish setup`, which requires **free − 0.2 GiB ≥ 1.5 GiB** (≈1.7 GiB free *while a run is in flight*). A run consumes ~0.24 GiB (fresh clone + adopted builds) and entered that check at 1.59 GiB — ~0.11 GiB short. Freeing **912 MiB** (stale pytest scratch + `/var/folders/_0/…/C/*` caches; `$TMPDIR/Roblox` was left alone — Roblox was running) put 2.48 GiB in flight and both runs PASSED. So the honest state of that first pair is **BLOCKED (insufficient disk), not FAIL** — but at that commit `clean-room.sh` labelled any non-zero step "FAIL" (flattening BLOCKED into FAIL) and deleted its own logs, keeping only a ≤6-line excerpt — **BUG-011, since FIXED in `255de46`** (a disk-blocked run is now BLOCKED, exit 3, logs kept; see the next paragraph). The two blocked records keep their FAIL label as written, with a correction note in `docs/clean-room-records/README.md`. All four records are kept in `docs/clean-room-records/` (`2026-09-19-run{1,2}-{182731,182956}.md` = the disk-blocked pair, `2026-09-19-run{1,2}-{183532,183806}.md` = the passing pair) and were **uncommitted** when this block was written.
> **Loading is now checkable (2026-09-19, load-everything session).** Goal restated by Wilson: experienced users on the FREE tools (Hermes, FreeBuff), but the real point is proving everything **loads properly**. Added load canaries (`manifests/load-canaries.json`, `docs/LOADING-CHECK.md`): a unique phrase in `AGENTS.md`, `CLAUDE.md` and both skills. `scripts/agent-check.sh` now drives **Hermes's own loaders**: `AGENTS.md`'s canary is in the context Hermes would inject (CLAUDE.md's is absent — Hermes loads ONE context file), both skills are served with the current canary (it caught a **stale user-level copy** before reinstall), scanners accept everything, and the **trusted-repo path works — tested in a throwaway `HERMES_HOME`, real config untouched** (real config still has no trusted dirs). **FreeBuff** cannot be scripted (no non-interactive mode): `agent-check` reports UNRESOLVED and points to the canary prompt; a FreeBuff session was seen running `new-run` and writing a correct handoff, which is evidence but not a canary test. **BUG-011 fixed:** `clean-room.sh` now labels a disk-blocked run BLOCKED (exit 3), skips the heavy steps, and keeps the logs; the two earlier disk-blocked records keep their FAIL label with a correction note in `docs/clean-room-records/README.md`. Tests: **67** runtime tests (2 slow-tier skipped), `agent-check` 36 PASS / 1 WARN (repo not trusted in Hermes: Wilson's call) / 0 FAIL / 1 UNRESOLVED (FreeBuff live).
> **Safe to wipe and re-download (2026-09-19, checked; see `docs/RESTORE.md`).** Wilson is verifying that everything can be deleted and re-cloned. Verified: `~/fcve` and `~/ico-collatz/ico_collatz_verification` are identical to their private remotes (0 unpushed, 0 stashes). **Found and fixed the one real hole:** `~/ico-collatz` is NOT a git repo and held files the delivered ledgers cite (no hashes: irreplaceable). 77 small files (368 KB) are now copied unchanged into `legacy-evidence/` with `MANIFEST.sha256` + a test that every cited file has its copy (originals untouched; exports/targets/PDFs excluded). `scripts/restore.sh` puts back the gitignored source PDFs verified by sha256 (refuses any other file); `clean-room.sh` rehearses restore → setup → doctor → smoke → audit: PASS, no engine test skipped. **Still not covered by any backup:** the 7.7 GB `~/ico-collatz/targets/`, 1.1 GB `experiments/` (exports, regenerable), Hermes/FreeBuff settings, Vault notes, memory. A from-scratch tool build is still unrehearsed (~3.5 GiB free needed). Tests: **75** (2 slow-tier skipped).
> **Open, in the order I'd look at them (nothing is urgent):**
> 1. A fresh Claude session in a fresh clone reconstructs the environment from README/CLAUDE.md alone (needs a person) — still the only way to close the clean room's own caveat. 2. ~~Two more clean-room runs at the newest commit~~ **DONE 2026-09-19: 2 PASS at `8b3b416` (tools adopted, no fresh Claude session)**; still open: `scripts/run-tests.sh --full` (2 slow-tier tests skipped), failure injection by hand, a from-scratch tool build (~3.5 GiB), and a decision on the candidate BUG-011 mislabel above.
> 3. Wilson's calls: `hermes skills trust /Users/lordwilson/fcve` (security setting, not run); ~~pick a LICENSE~~ **done: Apache-2.0** (`LICENSE`, `NOTICE`); the plaintext Telegram token in `~/.hermes/config.yaml`; free disk (**2.7 GiB free now**, 82%, after freeing 912 MiB on 2026-09-19; a from-scratch tool build needs ≈3.5 GiB; `~/msb-backups/` grows ~1 GB/day, so the clean-room floor of ~1.7 GiB in flight will be hit again).
...
> Memory/Vault mirrors: `~/Documents/Vault/40_Memory/SESSION-HANDOFF-2026-09-19-final.md` (current; mirrors this block), `~/Documents/Vault/40_Memory/SESSION-HANDOFF-2026-09-19.md` (earlier running log, kept), `~/.claude/projects/-Users-lordwilson/memory/project_fcve_state.md`.
