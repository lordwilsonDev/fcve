...
- `2026-09-19-run1-185521.md` — **PASS** on commit `255de46` (load canaries, Hermes loader proof, BUG-011 fix). Same caveats: adopted tools, scripted, no fresh Claude session.
- `2026-09-19-run1-191526.md` — **PASS** on commit `c85fb9d`+ (adds `scripts/restore.sh`; first run where the source PDFs are restored from the pinned upstream repo by sha256 and **no engine test suite is skipped**). Same caveats: adopted tools, scripted, no fresh Claude session.
- `2026-09-19-run1-203928.md` — **PASS on commit `5e9abb5`, tools built FROM SCRATCH** (no `--reuse-from`; setup 139 s; restore.sh restored the source PDFs; no engine suite skipped). First run that does not adopt existing tool builds; resolves that caveat for one run. Still: scripted, no fresh Claude session, one run at this commit.
