...
31. **Give "did it load?" a checkable answer.** File existence proves nothing about what an agent read. A unique canary phrase in each instruction file and skill (`manifests/load-canaries.json`) lets a person, or a script driving the agent's own loader, verify loading without trusting a model's self-report. It also caught a stale user-level skill copy that Hermes was silently serving.
32. **Test the risky-to-enable path in a throwaway profile.** Trusting a repo in Hermes is a security decision, but "would the skills load if trusted?" can be answered with `HERMES_HOME=<tmp>` without touching the real config.
