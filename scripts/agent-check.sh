...
SKILL="$FCVE_ROOT/skills/verifying-lean-proofs"; NAME=verifying-lean-proofs
...
python3 - "$SKILL" "$NAME" <<'PY' && record PASS skill frontmatter "name matches its directory, is a valid slug (<=64), description <=1024 chars (FreeBuff's limits, read from its binary)" || record FAIL skill frontmatter "SKILL.md frontmatter would be rejected (see message above)"
import re, sys
...
drift() { diff -rq -x .DS_Store "$SKILL" "$1" >/dev/null 2>&1; }
...
if have hermes; then
  ...
  if [ -d "$D" ]; then drift "$D" && record PASS hermes "user-level skill" "$D identical to the repo copy (loads without a trust step)" || record WARN hermes "user-level skill" "$D differs from the repo copy: run scripts/install-agent-skills.sh --agent hermes"
  else record WARN hermes "user-level skill" "not installed: scripts/install-agent-skills.sh --agent hermes (or trust this repo so its .agents/skills loads)"; fi
  if hermes skills list 2>/dev/null | grep -qi "verifying-lean-pro"; then record PASS hermes "skill listed" "\`hermes skills list\` shows verifying-lean-proofs"
  else record WARN hermes "skill listed" "\`hermes skills list\` does not show it (install the user-level copy or trust the repo)"; fi
  # Deterministic proof of what Hermes loads (offline, no model): context bytes in the repo vs a neutral directory, and the skill index.
  PSR=$(cd "$FCVE_ROOT" && hermes prompt-size --json 2>/dev/null); PSN=$(cd /tmp && hermes prompt-size --json 2>/dev/null)
  ...
  echo "$PSR" | python3 -c "import sys,json;d=json.load(sys.stdin);sys.exit(0 if any(x.get('name')=='$NAME' if isinstance(x,dict) else x[0]=='$NAME' for x in d['skills_breakdown']) else 1)" 2>/dev/null \
    && record PASS hermes "skill in prompt index" "prompt-size skills_breakdown includes $NAME" || record WARN hermes "skill in prompt index" "$NAME is not in Hermes's skills index (install the user-level copy or trust the repo)"
  if [ -x "$HERMES_PY" ]; then
...
r = sg.scan_skill(root / "skills" / "verifying-lean-proofs", source="local")
print("SKILL", r.verdict, len(r.findings))
PY
)
    ...
    V=$(echo "$RES" | sed -n 's/^SKILL \([a-z]*\).*/\1/p')
    case "$V" in safe) record PASS hermes "skills_guard verdict" "safe (a 'dangerous' verdict would quarantine the skill in a trusted project)" ;;
      caution) record WARN hermes "skills_guard verdict" "caution: loads, but review the findings" ;;
      dangerous) record FAIL hermes "skills_guard verdict" "dangerous: Hermes would quarantine the skill in a trusted project" ;;
      *) record UNRESOLVED hermes "skills_guard verdict" "could not run Hermes's scanner: $(echo "$RES" | tail -1 | cut -c1-120)" ;; esac
...
if have freebuff; then
  ...
  for UD in "$HOME/.agents/skills/$NAME" "$HOME/.claude/skills/$NAME"; do
    if [ -d "$UD" ]; then drift "$UD" && record PASS freebuff "user-level skill $(dirname "$UD" | sed "s#$HOME#~#")" "identical to the repo copy" || record WARN freebuff "user-level skill $(dirname "$UD" | sed "s#$HOME#~#")" "differs from the repo copy: scripts/install-agent-skills.sh"
    else record WARN freebuff "user-level skill $(dirname "$UD" | sed "s#$HOME#~#")" "not installed: scripts/install-agent-skills.sh --agent $( [ "${UD#*/.agents/}" != "$UD" ] && echo freebuff || echo claude)"; fi
  done
